...
function CameraRig() {
  const {camPos, setCamPos} = useContext(CamContext);
  const easedPos = useRef(0);

  useEffect(() => {
    const onScroll = () => setCamPos(window.scrollY)
  ...
  // })
  useFrame(({ camera }) => {
    camera.position.x = 5 - camPos * -0.01
    camera.position.z = 5 - camPos * -0.02
  })
  // useFrame((_, delta) => {
  //   const ease = 1 - Math.exp(-6 * delta)
  //   easedPos.current += (camPos - easedPos.current) * ease
